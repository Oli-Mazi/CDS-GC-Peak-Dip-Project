# -*- coding: utf-8 -*-
"""
Created on Mon May 26 14:26:52 2025
...
def Codon_GC_Definer(genome):
    ...
    for gene in genedict.values():
        ...
        for p in range(min(500, len(seq) // 3)):
            GCcount= seq[(p*3)+1].count("G")+seq[(p*3)+1].count("C")
            gccontent[p]+=GCcount
            lengthlist[p]+=1
...
metagenedf.to_csv(f"D:/Archaea_Summative_GC_3end/3end_{group2}_GC2.csv", index=False)