# -*- coding: utf-8 -*-
"""
Created on Sun May 25 09:57:17 2025
...
GC4set={"CT","GT","CC","AC","GC","CG", "GG", "TC"}
...
def Codon_GC_Definer(genome):
    ...
    for gene in genedict.values():
        for p in range(min(500, len(gene.seq) // 3)):
            codon=gene.seq[(p*3):((p*3)+3)]
            if codon[0:2] in GC4set:
                GCcount = codon[2].count("G") + codon[2].count("C")
                gccontent[p]+=GCcount
                lengthlist[p]+=1
...
metagenedf.to_csv(f"D:/Bacteria_Summative_GC/{group2}_GC4.csv", index=False) 
