# -*- coding: utf-8 -*-
"""
Created on Wed May 14 13:11:39 2025
...
def Codon_GC_Definer(genome):
    ...
    for gene in genedict.values():
        for p in range(min(500, len(gene.seq) // 3)):
            GCcount= gene.seq[((p*3)+1)].count("G")+gene.seq[((p*3)+1)].count("C")
            gccontent[p]+=GCcount
            lengthlist[p]+=1
...
metagenedf.to_csv(f"D:/Bacteria_Summative_GC/{group2}_GC2.csv", index=False)
