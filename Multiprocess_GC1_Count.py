# -*- coding: utf-8 -*-
"""
Created on Fri May 23 09:56:37 2025
...
def Codon_GC_Definer(genome):
    ...
    for gene in genedict.values():
        for p in range(min(500, len(gene.seq) // 3)):
            GCcount= gene.seq[(p*3)].count("G")+gene.seq[(p*3)].count("C")
            gccontent[p]+=GCcount
            lengthlist[p]+=1
...
metagenedf.to_csv(f"D:/Bacteria_Summative_GC/{group2}_GC1.csv", index=False)
